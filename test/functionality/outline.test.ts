...
describe(`outline`, () => {
    test(`offsetting a straight horizontal line should work`, () => {
        const b = new Bezier({ x: 0, y: 0 }, { x: 2, y: 0 }, { x: 1, y: 0 }, { x: 3, y: 0 });
        const outline = b.outline(10);
        const coords = outline.map((b) => b.points);
    ...
    test(`offsetting a vertical straight line should work`, () => {
        const b = new Bezier({ x: 0, y: 0 }, { x: 0, y: 2 }, { x: 0, y: 1 }, { x: 0, y: 3 });
        const outline = b.outline(10);
        const coords = outline.map((b) => b.points);
        expect(coords).toEqual([
            [
    ...
    test(`offsetting a straight line at an angle should work`, () => {
        const b = new Bezier({ x: 0, y: 0 }, { x: 2, y: 2 }, { x: 1, y: 1 }, { x: 3, y: 3 });
        const outline = b.outline(10);
        const coords = outline.map((b) => b.points);
        expect(coords).toEqual([
            [
